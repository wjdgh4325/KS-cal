...
class ModelEvaluator_test_km(object):
    ...
    def _eval_phase(self, model, data_loader, phase, device):
        ...
        for src, tgt in data_loader:
            ...
            self._record_batch(num_both, loss, loss_meter=records['loss_meter'])
            num_evaluated += src.size(0)

            km_value = util.km_calibration2(pred_params, tgt, self.args)
            self._record_batch(num_both, km_value, loss_meter=records['km_meter'])  # 여기 추가
        ...
        metrics = self._get_summary_dict(phase,
                                         loss_meter=records['loss_meter'],
    ...
    @staticmethod
    def _get_summary_dict(phase, loss_meter=None, km_meter=None):
        metrics = {
            phase + '_loss': loss_meter.avg,
        }
        if km_meter is not None:
            metrics[phase + '_KMcal'] = km_meter.avg
        return metrics
